...
function Card({ children, className = "" }: { children: React.ReactNode; className?: string }) {
  return (
    <div className={`bg-white rounded-2xl border border-[#CDD5B0] p-6 transition-all duration-300 hover:-translate-y-1.5 hover:shadow-lg hover:shadow-[#5C6B3A]/10 ${className}`}>
      {children}
    </div>
  );
}

const SECTION_LABELS = ["Hero", "Programm", "Dresscode", "Anreise", "Anmeldung"];
...
export default function FullPagePreview() {
  ...
  return (
    <div style={{ position: "relative", fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
      ...
      {/* NAV */}
      <nav className="fixed top-0 left-0 right-0 z-50" style={{ background: "#1E2614", height: 64, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 24px" }}>
        <span />
        <div className="hidden sm:flex gap-6 text-sm font-medium">
          {[["Programm","1"],["Dresscode","2"],["Anreise","3"],["Anmeldung","4"]].map(([l, i]) => {
            const isActive = active === Number(i);
            return (
              <button key={l} onClick={() => scrollTo(Number(i))}
                style={{ color: isActive ? "#CDD5B0" : "rgba(205,213,176,0.5)", background: "none", border: "none", cursor: "pointer", fontSize: 14, fontWeight: isActive ? 700 : 400, padding: "4px 0", borderBottom: isActive ? "2px solid #5C6B3A" : "2px solid transparent", transition: "all 0.3s ease" }}>
                {l}
              </button>
            );
          })}
        </div>
        <a href="/preview" style={{ color: "rgba(205,213,176,0.5)", fontSize: 12 }}>← Zurück zur Preview</a>
      </nav>
      ...
      >
        ...
        {/* ── HERO ── */}
        <div ref={el => { sectionRefs.current[0] = el; }} style={sectionStyle}>
          <section className="flex flex-col items-center justify-center text-center px-6 py-16 flex-1">
            ...
            <div className="max-w-3xl mx-auto grid grid-cols-1 sm:grid-cols-3 gap-4 mt-12 w-full">
              <Card className="space-y-2 flex flex-col">
                ...
                <p className="text-sm" style={{ color: MUTED }}>Ab ca. 15:30 Uhr bis die Sterne zählen</p>
              </Card>
              <Card className="space-y-2 flex flex-col">
                <p className="text-2xl">📍</p>
                ...
                <p className="text-sm" style={{ color: MUTED }}>Reichenbachstrasse 161, 3004 Bern</p>
                <a href="https://maps.google.com/?q=Reichenbachstrasse+161+3004+Bern" target="_blank" rel="noopener noreferrer" className="text-sm font-semibold mt-auto" style={{ color: ACCENT }}>Google Maps →</a>
              </Card>
              <Card className="space-y-2 flex flex-col">
                <p className="text-2xl">📋</p>
                ...
                <p className="text-sm" style={{ color: MUTED }}>Damit die Küche weiss, wie viele Teller sie aufwärmen soll.</p>
                <button onClick={() => scrollTo(4)} className="text-sm font-semibold mt-auto text-left" style={{ color: ACCENT, background: "none", border: "none", cursor: "pointer" }}>Jetzt anmelden →</button>
              </Card>
            </div>
          </section>
        </div>
        ...
        {/* ── PROGRAMM ── */}
        <div ref={el => { sectionRefs.current[1] = el; }} style={sectionStyle}>
        ...
        {/* ── DRESSCODE ── */}
        <div ref={el => { sectionRefs.current[2] = el; }} style={sectionStyle}>
          <section className="px-6 py-12 flex-1">
            <div className="max-w-3xl mx-auto">
                ...
                {[
                  ...
                  <Card key={title} className="h-full">
                    ...
                    <ul className="space-y-1.5">
                      {items.map(item => (
                        <li key={item} className="text-sm flex gap-2" style={{ color: MUTED }}>
        ...
        {/* ── ANREISE ── */}
        <div ref={el => { sectionRefs.current[3] = el; }} style={sectionStyle}>
          <section className="px-6 py-12 flex-1">
            <div className="max-w-3xl mx-auto">
        ...
        {/* ── ANMELDUNG ── */}
        <div ref={el => { sectionRefs.current[4] = el; }} style={sectionStyle}>
          <section className="px-6 py-12 flex-1">
      ...
      <div style={{ position: "fixed", right: 20, top: "50%", transform: "translateY(-50%)", display: "flex", flexDirection: "column", gap: 8, zIndex: 50 }}>
        {SECTION_LABELS.map((label, i) => (
          <button key={label} onClick={() => scrollTo(i)} title={label}
            style={{
              width: 4,
